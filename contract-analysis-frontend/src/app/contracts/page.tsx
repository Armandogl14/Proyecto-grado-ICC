...
export default function ContractsPage() {
  ...
  const getStatusInfo = (status: string) => {
    switch (status) {
      case 'pending': return { icon: <FileClock className="w-4 h-4" />, text: 'Pendiente', color: 'text-yellow-600' }
      case 'analyzing': return { icon: <Loader2 className="w-4 h-4 animate-spin" />, text: 'Analizando', color: 'text-purple-600' }
      case 'completed': return { icon: <CheckCircle className="w-4 h-4" />, text: 'Completado', color: 'text-green-600' }
      case 'error': return { icon: <AlertTriangle className="w-4 h-4" />, text: 'Error', color: 'text-red-600' }
      default: return { icon: <FileText className="w-4 h-4" />, text: 'Desconocido', color: 'text-gray-500' }
    }
  }
  ...
  return (
    <>
      ...
      <main className="container mx-auto p-4 md:p-8">
        ...
        {/* Filters */}
        <Card className="bg-card/80 border-border/60 backdrop-blur-sm p-5 rounded-2xl mb-8">
          <div className="grid gap-4 md:grid-cols-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-2.5 bg-secondary/70 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-foreground placeholder:text-muted-foreground"
              ...
              <Button
                ...
                className="border-border hover:bg-secondary h-full"
              >
        ...
        <div className="space-y-4">
          {isLoading ? (
             [...Array(5)].map((_, i) => (
                <Card key={i} className="bg-card/80 border-border/60 p-5 rounded-2xl animate-pulse h-24"></Card>
             ))
            ...
            contractsData.results.map((contract: Contract) => {
              ...
              return (
              <Card key={contract.id} className="bg-card/80 border-border/60 backdrop-blur-sm rounded-2xl hover:border-border transition-colors">
                <CardContent className="p-4 md:p-5">
                  <div className="grid md:grid-cols-4 items-center gap-4">
                      ...
                      <div className="md:col-span-2">
                        ...
                        <p className="text-sm text-muted-foreground">{contract.contract_type.name}</p>
                      </div>
                      ...
                      <div className="flex items-center justify-end gap-4">
                         <div className="text-right hidden lg:block">
                            <p className="text-sm text-foreground">Creado</p>
                            <p className="text-xs text-muted-foreground">{formatDate(contract.created_at)}</p>
                         </div>
                         <Link href={`/contracts/${contract.id}`}>
                           <Button size="icon" variant="ghost" className="text-muted-foreground hover:bg-secondary hover:text-foreground">
             ...
             <Card className="bg-card/80 border-border/60 p-12 rounded-2xl text-center">
                <FileText className="w-12 h-12 text-muted-foreground/50 mx-auto mb-4" />
                <h3 className="text-xl font-semibold text-foreground">No se encontraron contratos</h3>
                <p className="text-muted-foreground mt-2 mb-6">Prueba a ajustar los filtros o crea tu primer contrato.</p>
                <Button onClick={handleClearFilters} variant="outline" className="border-border">Limpiar filtros</Button>
            </Card>
          )}
        </div>
